...
namespace bio {
    ...
    std::u16string BinaryIO::readWChar2Byte(const size_t size,
                                            const ByteOrder endian) {
        std::u16string result(size, u'\0');
        std::memcpy(&result[0], this->mData, size * sizeof(char16_t));
        this->mData += size * sizeof(char16_t);
    ...
    std::u16string BinaryIO::readWChar2ByteNT(const ByteOrder endian) {
        ...
        this->mData += (len + 1) * sizeof(char16_t);
        return result;
    }

    void BinaryIO::writeWChar2Byte(const std::u16string &input,
                                   const ByteOrder endian,
    ...
    std::u32string BinaryIO::readWChar4Byte(const size_t size,
                                            const ByteOrder endian) {
        std::u32string result(size, u'\0');
        std::memcpy(&result[0], this->mData, size * sizeof(char32_t));
        this->mData += size * sizeof(char32_t);
    ...
    std::u32string BinaryIO::readWChar4ByteNT(const ByteOrder endian) {
        ...
        this->mData += (len + 1) * sizeof(char32_t);
        return result;
    }

    void BinaryIO::writeWChar4Byte(const std::u32string &input,
                                   const ByteOrder endian,
                                   const bool nullTerminate = false) {
